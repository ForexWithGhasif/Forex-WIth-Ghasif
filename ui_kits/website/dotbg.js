/* ==========================================================================
   ...
   per frame cheaply with a plain loop. This replaces the previous WebGL
   shader background, so the Three.js CDN script has been removed from every
   page too; nothing else on the site used it.
...
(function () {
  ...
  var CONFIG = {
    ...
    // CSS custom property the dots are tinted from (see tokens/colors.css).
    colorVar: { name: '--gold-400', fallback: '#D6AF43' }
  };
  ...
  function boot() {
    ...
    var dotRgb = readBrandRgb(CONFIG.colorVar);
    ...
    var dots = []; // each: {bx, by, phase, rx, ry} — base pos, phase seed, current repel offset
    var w = 0, h = 0;
    ...
    /* ---------------------------------------------------------------------
       ...
       grid position with no float/pulse/repel.
       --------------------------------------------------------------------- */
    function drawFrame(t, animated) {
      ctx.clearRect(0, 0, w, h);
      ctx.fillStyle = 'rgba(' + dotRgb + ',1)';
